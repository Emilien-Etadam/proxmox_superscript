...
# Paramètres :
#   $1 — libellé court
#   $2 — URL HTTPS du script raw
#   $3 — (optionnel) "skip-stopped" pour patcher clean-lxcs (ignorer les CT non running)
...
run_community_script() {
    ...
    if [[ "$patch_mode" == "skip-stopped" ]]; then
        echo "[!] Patch lxc-postconf : les CT non running seront ignorés (pas de pct start)."
    fi
    ...
    if [[ "$rc" -eq 0 ]]; then
        ...
        if [[ "$rc" -eq 255 ]]; then
            ...
            echo "[!] Astuce : cochez-le pour l'exclure dans whiptail, ou utilisez uniquement des CT démarrés."
        else
...
# Lance clean-lxcs.sh (nettoyage + apt update) en ignorant les CT non running.
# Retour : toujours 0 (le menu principal ne doit pas s'interrompre sous set -e).
run_community_clean_and_update_lxcs() {
    run_community_script "clean-and-update-lxcs" "$COMMUNITY_CLEAN_LXCS_URL" "skip-stopped" || true
}
