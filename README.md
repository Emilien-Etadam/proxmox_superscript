...
| **2** | Clean and update LXC | Exécute [`clean-lxcs.sh`](https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/tools/pve/clean-lxcs.sh) après un patch lxc-postconf : les CT **non running sont ignorés** (pas de `pct start`). Nettoyage multi-CT puis `apt update`. |
